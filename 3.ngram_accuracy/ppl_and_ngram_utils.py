...
def load_model(model_path, device, model_type="base"):
    ...
    if model_type == "multimodal":
        ...
        if "qwen" in model_path.lower():
            ...
            # Keep BLIP compatibility
            processor = BlipProcessor.from_pretrained(model_path, trust_remote_code=True)
            model = BlipForConditionalGeneration.from_pretrained(model_path, trust_remote_code=True)
...
################################################################
def load_data_from_jsonl(jsonl_file_name, num_samples=None, ngram=False, model_type="base"):
    ...
    if model_type == "multimodal":
        return load_data_with_image(jsonl_file_name, num_samples, ngram)
    ...
    random.seed(666)
    # when num_samples <=0 or None, load all, otherwise random sample
...
def calculate_n_gram_accuracy(n, k, dataset, model, tokenizer, device, output_file, model_type = "base"):
    ...
    for question, answer, choice in zip(dataset['question'], dataset['answer'], dataset['choice']):
        ...
        tokens = tokenizer.tokenizer.tokenize(format_text)
    ...
    for idx in tqdm(range(0, len(dataset['question']))):
        ...
        sample = tokenizer.tokenizer.convert_tokens_to_string(tokens)
        ...
        for start_index in starting_points:
            ...
            prompt = tokenizer.tokenizer.convert_tokens_to_string(prefix_tokens)
            if model_type == "chat":
                prompt = tokenizer.build_inputs_with_special_tokens(prompt)
            if is_mm:
                # Multimodal scenario: insert image_token before text as placeholder to trigger correct expansion
                # Convert prefix_tokens to text
                prompt_text = tokenizer.tokenizer.convert_tokens_to_string(prefix_tokens)
                if model_type == "chat":
                    prompt_text = tokenizer.build_inputs_with_special_tokens(prompt_text)
                # Get the image_token defined by the processor
                image_token = tokenizer.image_token
                prompt_with_image = f"{image_token} {prompt_text}"
                # Open the image (relative path) and process it
                img_path = dataset.get("image_path", [])[idx]
                img = Image.open(img_path).convert("RGB")
                # Call the processor with explicit images and text
                encoding = tokenizer(
                    images=img,
                ...
                # Retain original logic for pure text scenario
                prompt_text = tokenizer.tokenizer.convert_tokens_to_string(prefix_tokens)
                if model_type == "chat":
                    prompt_text = tokenizer.build_inputs_with_special_tokens(prompt_text)
                # Tokenize text-only prompts
            encoding = tokenizer(
                    prompt_text,
            ...
            original_ids = tokenizer.tokenizer.convert_tokens_to_ids(tokens[start_index: start_index + n])
...
# Add multimodal dataset loader for TSV with image_path
def load_data_with_image(path, num_samples=None, ngram=False):
    import csv, random
    ds = {"question": [], "choice": [], "answer": [], "image_path": []}
    with open(path, "r", newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f, delimiter="\t")
        rows = list(reader)
    random.seed(666)
    # Load all when num_samples <= 0 or None; otherwise sample randomly
    if num_samples is None or num_samples <= 0:
        selected = rows
    else:
        selected = random.sample(rows, min(num_samples, len(rows)))
    for item in selected:
        q = item.get("question", "")
        opts = {k: item[k] for k in ["A","B","C","D"] if k in item and item[k]}
        choice_str = "".join(f"\n{k}. {v}" for k, v in opts.items())
        ds["question"].append(q)
        ds["choice"].append(choice_str)
        ds["answer"].append(item.get("answer", ""))
        # Convert absolute path to relative (remove leading slash)
        raw_path = item.get("image_path", "")
        ds["image_path"].append(raw_path.lstrip("/"))
    return ds