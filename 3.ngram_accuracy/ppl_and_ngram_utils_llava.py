...
from llava.constants import IMAGE_TOKEN_INDEX, IMAGE_PLACEHOLDER


def load_model(model_path, device, model_type="base"):
    if model_type == "multimodal":
        # For LLaVA multimodal models
        ...
        # For Qwen2VL instruction (legacy)
...
def calculate_n_gram_accuracy(n, k, dataset, model, tokenizer, device, output_file, model_type = "base", image_processor=None):
    """
    ...
    for idx in tqdm(range(0, len(dataset['question']))):
        ...
        for start_index in starting_points:
            ...
            if is_mm:
                # Multimodal scenario: use LLaVA if available
                if image_processor is not None:
                    # Convert prefix to text and add image placeholder
                    prefix_text = tokenizer.convert_tokens_to_string(prefix_tokens)
                    if model_type == "chat":
                        prefix_text = tokenizer.build_inputs_with_special_tokens(prefix_text)
                    ...
                    ids = tokenizer_image_token(prompt_with_image, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt")
                    ...
                    prompt_text = tokenizer.convert_tokens_to_string(prefix_tokens)
                    if model_type == "chat":
