...
def worker(model_name_or_path,
    ...
    while True:
        ...
        print(f"[WORKER {device}] Received {'canonical' if is_canonical else 'shuffled'} tasks for shard {shard_id}", flush=True)

        if tokens is None: # Quit.
            break

        print(f"[WORKER {device}] Computing logprob for shard {shard_id}, items={len(tokens) if isinstance(tokens, list) else 'text'}", flush=True)
        # Compute logprob; if tokens is a list (multi-sample shard), sum each
        if model_name_or_path and isinstance(tokens, list) and model_type in ('multimodal','llava'):
            total_logp = 0.0
            for enc in tokens:
                total_logp += compute_logprob_of_encoding(enc, m, context_len, stride, device)
            logprob = total_logp
        else:
            logprob = compute_logprob_of_token_sequence(tokens, 
                                                        ...
                                                        stride,
                                                        device=device)
        print(f"[WORKER {device}] Computed logprob {logprob} for shard {shard_id}", flush=True)
...
def main(model_name_or_path,
    ...
    if model_type in ('multimodal', 'llava') and dataset_format == 'tsv':
        ...
        for item in examples:
            ...
            all_encs.append(processor(text=[text_input], images=[img], return_tensors='pt', padding=True))
        # 把示例均分到 num_shards 个 shard
        base = num_examples // num_shards
        rem = num_examples % num_shards
        shard_counts = [base+1 if i<rem else base for i in range(num_shards)]
        cum = np.cumsum([0] + shard_counts)
        tokenized_examples = [all_encs[cum[i]:cum[i+1]] for i in range(num_shards)]
        shard_items = [examples[cum[i]:cum[i+1]] for i in range(num_shards)]
    else:
    ...
    while num_ready < num_workers:
        ...
        print(f"GPU {gpu_id} loaded model.", flush=True)
        num_ready += 1
    
    # All GPUs ready, start dispatch
    print(f"[INFO] All GPUs ready. num_shards={num_shards}, permutations_per_shard={permutations_per_shard}", flush=True)
    ...
    if model_type in ('multimodal', 'llava'):
        ...
        # Dispatch每个 shard 的 batch encoding
        for i in range(num_shards):
            print(f"[DEBUG] Dispatching canonical for shard {i}", flush=True)
            encs = tokenized_examples[i]
            worker_queues[0].put((encs, i, True))
            # 洗牌：对每个 sample 内部打乱选项后重新编码
            for j in range(permutations_per_shard):
                w = j % num_workers
                print(f"[DEBUG] Dispatching shuffle {j} for shard {i} to worker {w}", flush=True)
                shuffled_encs = []
                for item in shard_items[i]:
                    img_path = item['image_path'].lstrip('/')
                    img = Image.open(img_path).convert('RGB')
                    question = item.get('question','')
                    options = [item[k] for k in ['A','B','C','D','E'] if k in item]
                    sh_opts = random.sample(options, k=len(options))
                    content = [{'type':'image','image':img_path},{'type':'text','text':question}]
                    for opt in sh_opts: content.append({'type':'text','text':opt})
                    msgs = [{'role':'user','content':content}]
                    txt = processor.apply_chat_template(msgs, tokenize=False, add_generation_prompt=True)
                    shuffled_encs.append(processor(text=[txt], images=[img], return_tensors='pt', padding=True))
                worker_queues[w].put((shuffled_encs, i, False))
    else:
